        ...
        stage('GetPrimaryTaskSet'){
            steps{
                script{
                    ...
                    script: "/usr/local/bin/aws ecs --region us-west-1 describe-services --services $SERVICE_ARN --cluster $CLUSTER_ARN",
                    returnStdout: true
                    ).trim()
        ...
        stage('RegisterTaskDefinition') {
            steps {
                ...
                script {
                    ...
                    script: "/usr/local/bin/aws ecs --region us-west-1 register-task-definition --cli-input-json file://${taskDefFile}",
                    returnStdout: true
                    ).trim()
        ...
        stage('CreateTaskSetTemplate') {
            steps{
                script{
                    ...
                    script: "/usr/local/bin/aws ecs --region us-west-1 create-task-set --service $SERVICE_ARN --cluster $CLUSTER_ARN --cli-input-json file://${taskSetFile}",
                    returnStdout: true
                    ).trim()
        ...
        stage('EnableTestListener'){
            steps{
                script{
                    ...
                    script: "/usr/local/bin/aws elbv2 --region us-west-1 modify-listener --listener-arn $TEST_LISTENER_ARN --cli-input-json file://${testDefaultActionsFile}",
                    returnStdout: true
                    ).trim()
        ...
        stage('SwapLive'){
            steps{
                script{
                    ...
                    script: "/usr/local/bin/aws elbv2 --region us-west-1 modify-listener --listener-arn $LIVE_LISTENER_ARN --cli-input-json file://${liveDefaultActionsFile}",
                    returnStdout: true
                    ).trim()
            ...
            steps{
                script{
                    ...
                    def updatePrimaryTaskSetOutput = sh (
                        script: "/usr/local/bin/aws ecs --region us-west-1 update-service-primary-task-set --service $SERVICE_ARN --cluster $CLUSTER_ARN --primary-task-set ${createTaskSetOutput.taskSet.taskSetArn}",
                        returnStdout: true
                        ).trim()
        ...
        stage('DeleteDeployment'){
            steps{
                script{
                    ...
                    script: "/usr/local/bin/aws ecs --region us-west-1 delete-task-set --cluster $CLUSTER_ARN --service $SERVICE_ARN --task-set ${primaryTaskSetJson.id}",
                    returnStdout: true
                    ).trim()
                    ...
                    script: "/usr/local/bin/aws ecs --region us-west-1 deregister-task-definition --task-definition ${primaryTaskSetJson.taskDefinition}",
                    returnStdout: true
                    ).trim()
